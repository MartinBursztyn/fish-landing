...
/* Features */
const FeatureRow = ({ img, alt, body, reversed, extraClass }) => (
  <div className={`feature-row ${reversed ? "reversed" : ""} ${extraClass || ""}`}>
...
const WhatWeGenerate = () => {
  ...
  const CARD_WIDTH = 320, GAP = 24, STEP = CARD_WIDTH + GAP;
  const [visible, setVisible] = useState(() =>
    typeof window !== "undefined" && window.innerWidth <= 700 ? 1 : 3
  );
  useEffect(() => {
    const onResize = () => setVisible(window.innerWidth <= 700 ? 1 : 3);
    ...
    <Section className="wwg-wrap">
      <Container>
        ...
        <div
          ...
          style={{ maxWidth: visible === 3 ? `${3*CARD_WIDTH + 2*GAP}px` : `${CARD_WIDTH}px` }}
        >
          <div className="wwg-track" style={{ transform:`translateX(${-index*STEP}px)` }}>
            {items.map((it,i)=>(
              <div className="wwg-card-wrap" key={i}>
                <div className="wwg-card">
                  ...
                  <div className="wwg-label">
                    {it.label.split("\n").map((line,j)=>(
                      <span key={j}>{line}{j===it.label.split("\n").length-1?"":<br/>}</span>
                    ))}
                  </div>
                </div>
              </div>
      ...
      <button className="wwg-arrow wwg-arrow--left" onClick={prev} aria-label="anterior">←</button>
      <button className="wwg-arrow wwg-arrow--right" onClick={next} aria-label="siguiente">→</button>
    </Section>
  );
};
...
const GiantButton = () => (
  <Section className="giant-cta">
    <Container>
      <a className="giant-btn" href="https://frontend-fisa.vercel.app" target="_blank" rel="noreferrer">
        PRUEBANOS
      </a>
    </Container>
  </Section>
  ...
  <Section className="footer-gradient">
    <Container>
      ...
      <div className="contact-list">
        <div className="contact-item"><span className="badge">in</span><span className="contact-text">FISA</span></div>
        <div className="contact-item"><span className="badge">@</span><span className="contact-text">FISA@gmail.com</span></div>
      </div>
    </Container>
  </Section>
);
